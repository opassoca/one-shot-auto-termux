...
class WPSpin:
    ...
    def _getDatabasePins(self, bssid: str) -> list:
        """Fetch pins from local database pins.csv based on BSSID prefix."""
        import os
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        db_path = os.path.join(project_root, 'pins.csv')
        if not os.path.exists(db_path):
            return []
        
        pins = []
        mac_prefix = bssid.replace(':', '').upper()[:6]
        try:
            with open(db_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if len(parts) >= 2:
                        db_pin = parts[0]
                        db_mac = parts[1].replace(':', '').upper()
                        if db_mac.startswith(mac_prefix):
                            clean_pin = db_pin.zfill(8)
                            if clean_pin not in pins:
    ...
    def _getSuggested(self, bssid: str):
        ...
        db_pins = self._getDatabasePins(bssid)
        res = []

        # Add Database PINs first
        for i, pin in enumerate(db_pins):
            item = {
                'id': f'db_pin_{i}',
                'name': f'Database PIN ({i+1})',
                'pin': pin
            }
            res.append(item)
        ...
        for identification in algos:
            ...
            if item['pin'] not in [r['pin'] for r in res]:
                res.append(item)
        return res
