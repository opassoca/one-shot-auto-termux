...
class Initialize:
    ...
    def singleConnection(self, bssid: str = None, pin: str = None, pbc_mode: bool = False, store_pin_on_fail: bool = False) -> bool:
        ...
        if self.CONNECTION_STATUS.STATUS == 'GOT_PSK':
            ...
            # Automatically connect on Android, or if -s/--save is specified
            if src.utils.isAndroid() or args.save:
    ...
    def _handle_wps_messages(self, line: str) -> bool:
        ...
        if 'M2D' in line:
            self.M2D_COUNT += 1
            logger.warning(f'Received WPS Message M2D ({self.M2D_COUNT}/5)')
            if self.M2D_COUNT >= 5:
                src.utils.die('AP is not ready after 5 attempts. Aborting.')
            
            self.CONNECTION_STATUS.STATUS = 'WPS_FAIL'
            self.CONNECTION_STATUS.IS_LOCKED = True
            logger.error('Error: AP is not ready yet, retrying in 2s...')
            time.sleep(2)
            return False
        ...
        if 'Building Message M' in line:
            ...
            self.IDENTITY_COUNT = 0 # Progress made, reset counter
    ...
    def _wpsConnection(self, bssid: str = None, pin: str = None,
        ...
        while True:
            ...
            if self.CONNECTION_STATUS.STATUS == 'RETRY_M2D':
                self.CONNECTION_STATUS.clear()
                self._sendAndReceive(cmd)
                continue
